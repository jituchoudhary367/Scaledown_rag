"""
RAG + Map-Reduce Summarization Pipeline - Main entry point
Flow: PDF → Chunks → Embeddings → FAISS → Retriever → Map-Reduce Summary → LLM → Metrics
"""
import os
import sys
import json
...
from src.rag_pipeline.summary_rag import run_summary_rag
...
def main():
    print("=" * 60)
    print("  RAG + Map-Reduce Summarization Pipeline")
    print("=" * 60)
    ...
    for i, question in enumerate(questions, 1):
        ...
        try:
            answer, latency, tokens = run_summary_rag(question, retriever)
    ...
    output_file = os.path.join(os.path.dirname(LOG_PATH), "summarization_results.json")
    with open(output_file, "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n{'=' * 60}")
    print(f"  Summarization RAG complete. Results → {output_file}")
    print(f"{'=' * 60}")
