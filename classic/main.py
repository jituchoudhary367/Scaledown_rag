"""
Classic RAG Pipeline - Main entry point
Flow: PDF → Chunks → Embeddings → FAISS → Retriever → LLM → Metrics
"""
import os
import sys
import json
...
from src.config import PDF_PATH, QUESTIONS_PATH, LOG_PATH
...
def main():
    ...
    for i, question in enumerate(questions, 1):
        ...
        try:
            answer, latency, tokens = run_classic_rag(question, retriever)
        ...
        print(f"    Latency: {latency['total_latency']:.2f}s | Tokens: {tokens['total_tokens']} | Quality: {quality_score:.3f}")
    ...
    output_file = os.path.join(os.path.dirname(LOG_PATH), "classic_results.json")
    with open(output_file, "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n{'=' * 60}")
    print(f"  Classic RAG complete. Results → {output_file}")
    print(f"{'=' * 60}")
